...
export function activate(context: vscode.ExtensionContext) {
    ...
    let disposable = vscode.commands.registerCommand('extension.dtomaker', () => {
        ...
        const sample : vscode.QuickPickItem[] = [];
        sample.push({ label : "$(alert)TESTTEST", description : "aaaaa" });
        sample.push({ label : "$(alert)bbbbbbbb", description : "ddddd" });
        vscode.window.showQuickPick(sample);
